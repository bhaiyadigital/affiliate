...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $pageMeta = Content::where('module', 'pages')->where('slug', 'index')->first();
    ...
    public function filter(Request $request)
    {
        ...
        if ($request->filled('share_ids') && $request->filled('types')) {
            ...
            if ($request->filled('concern')) {
                $assetQuery->whereHas('project', fn($q) => $q->where('parent_id', $request->concern));
                $campaignQuery->whereHas('project', fn($q) => $q->where('parent_id', $request->concern));
            }
        ...
        // Data for Sidebar
        $projects = Content::where('module', 'project')->where('status', 1)->get();
