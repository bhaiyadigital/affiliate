...
class AssetController extends Controller
{
    ...
    {
        $pageMeta = Content::where('module', 'pages')->where('slug', 'assets')->first();
        ...
        return view('assets.index', compact('assets', 'projects', 'assetTypes','pageMeta'));
    }

    public function create(Request $request)
    {
        $projects = Content::where('module', 'project')->where('status', 1)->orderBy('title')->get();
        $assetTypes      = AssetType::orderBy('name')->get();
    ...
    public function update(Request $request, Asset $asset)
    {
        ...
        $old = $asset->only(['title', 'asset_id_code', 'project_id', 'asset_type_id', 'meta_title', 'meta_description', 'meta_keywords']);
        $asset->update($validated);
        $new = $asset->only(['title', 'asset_id_code', 'project_id', 'asset_type_id', 'meta_title', 'meta_description', 'meta_keywords']);
        $this->syncMedia($request, $asset);
    ...
    private function validateAsset(Request $request, ?string $ignoreId = null): array
    {
        ...
        return $request->validate([
            ...
            'project_id' => ['required', 'exists:contents,id'],
