    ...
    <div class="lg:col-span-2 space-y-5">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input type="text" name="slug" id="slug"
                    value="{{ old('slug', $isEdit ? $campaign->slug : '') }}" placeholder="eid-special-offer-2025"
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div x-data="{
                ...
                {{-- Create --}}
                <x-drive-upload field-name="drive_file_id" :file-id="old(
                    'drive_file_id',
        ...
        <div
            ...
            <div>
                ...
                <select name="project_id" required
                    class="shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 @error('project_id') border-red-400 @enderror">

                    <option value="">— Select Project —</option>

                    {{-- কন্ট্রোলার থেকে আসা $projects (Content Model) লুপ করা হচ্ছে --}}
                    @foreach ($projects as $project)
                        <option value="{{ $project->id }}"
                            {{ old('project_id', $isEdit ? $campaign->project_id : '') == $project->id ? 'selected' : '' }}>
                            {{ $project->title }} {{-- এখানে $project->name এর বদলে $project->title ব্যবহার হয়েছে --}}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="status"
                    ...
                    <option value="draft"
                        {{ old('status', $isEdit ? $campaign->status : 'draft') === 'draft' ? 'selected' : '' }}>
                        Draft</option>
                    <option value="active"
                        {{ old('status', $isEdit ? $campaign->status : '') === 'active' ? 'selected' : '' }}>
                        Active</option>
                    <option value="expired"
                        {{ old('status', $isEdit ? $campaign->status : '') === 'expired' ? 'selected' : '' }}>
                        Expired</option>
                </select>
                @error('status')
                    <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
