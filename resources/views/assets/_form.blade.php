...
@push('scripts')
    ...
    <script>
        ...
        function initializeCKEditor() {
            const editorElement = document.querySelector('#asset-description');
...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
    ...
    <div class="lg:col-span-2 space-y-5">
        ...
        <div>
            ...
            <input type="text" name="title" id="title" required
                value="{{ old('title', $isEdit ? $asset->title : '') }}" placeholder="e.g. Static Banners:"
        ...
        <div>
            ...
            <input type="text" name="slug" id="slug" value="{{ old('slug', $isEdit ? $asset->slug : '') }}"
        ...
        <div>
            ...
            <div class="text-gray-900 dark:text-white">
                ...
                <textarea name="description" id="asset-description">{!! old('description', $isEdit ? $asset->description : '') !!}</textarea>
            </div>
        ...
        <div x-data="{
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-3">
                    @foreach ($asset->media as $media)
                        <div id="media-{{ $media->id }}"
                            class="relative group rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 aspect-video">
                            ...
                            @else
                                ...
                                <div class="absolute inset-0 flex items-center justify-center bg-black/30">
                                    <svg class="text-white" width="28" height="28" viewBox="0 0 20 20"
                                        fill="currentColor">
                                        <path
                            ...
                            @endif
                            <button type="button" @click="markDelete('{{ $media->id }}')"
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-3" x-show="mediaFiles.length > 0">
                <template x-for="(file, index) in mediaFiles" :key="index">
                    <div
                        class="relative group rounded-xl overflow-hidden border border-blue-200 dark:border-blue-700 bg-blue-50 dark:bg-blue-900/20 aspect-video">
                        ...
                        <template x-if="file.isVideo">
                            <div class="w-full h-full flex flex-col items-center justify-center gap-1">
                                <svg class="text-blue-400" width="28" height="28" viewBox="0 0 20 20"
                                    fill="currentColor">
                                    <path
                                        d="M6.3 2.841A1.5 1.5 0 004 4.11V15.89a1.5 1.5 0 002.3 1.269l9.344-5.89a1.5 1.5 0 000-2.538L6.3 2.84z" />
                                </svg>
                                <span class="text-xs text-blue-500 truncate px-2 w-full text-center"
            ...
            <div x-data="{
                ...
                uploadImage(image) {
                    return new Promise((resolve) => {
                        ...
                        xhr.open('POST', '{{ route('assets.media.upload-image') }}');
                        xhr.send(formData);
                    });
                },
                ...
                async remove(index) {
                    ...
                    if (img.driveId) {
                        ...
                        try {
                            await fetch('{{ route('assets.media.delete-temp-image') }}', {
                                method: 'POST',
                                headers: {
                        ...
                        <template x-for="(image, index) in images" :key="index">
                            <div class="relative rounded-lg border border-gray-200 bg-white p-2">
                                ...
                                <button type="button" @click="remove(index)"
                                    ...
                                    :class="(image.status === 'uploading' || image.status === 'removing') ?
                ...
                <input type="file" x-ref="imageInput" multiple accept="image/jpeg,image/png,image/webp,image/gif"
                    class="hidden" @change="addImages($event)">
            </div>
            ...
            <div>
                ...
                <div x-data="{
                    ...
                    <div @click="$refs.videoInput.click()" @dragover.prevent
                        ...
                        <div
                            class="w-10 h-10 rounded-full bg-purple-100 flex items-center justify-center dark:bg-purple-900/30">
                            <svg class="text-purple-500" width="20" height="20" viewBox="0 0 20 20"
                                fill="currentColor">
                                <path
                        ...
                        <div class="text-center">
                            <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Click or drag videos here
                            </p>
                            <p class="text-xs text-gray-400 mt-0.5">MP4, MOV, WEBM · Uploads directly to Google Drive
                            </p>
                        </div>
                    </div>
                            ...
                            <template x-for="(video, index) in videos" :key="index">
                                <div
                                    ...
                                    <div class="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                                        :class="{
                                            'bg-purple-100 dark:bg-purple-900/30': video.status === 'idle' || video
                                                .status === 'uploading',
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <template x-if="video.status === 'uploading'">
                                            <div class="mt-1.5">
                                                <div
                                                    class="w-full h-1.5 bg-gray-200 rounded-full dark:bg-gray-700 overflow-hidden">
                                                    <div class="h-1.5 bg-purple-500 rounded-full transition-all duration-300"
                                                        :style="'width: ' + video.progress + '%'"></div>
                                                </div>
                                                <p class="text-xs text-gray-400 mt-0.5 flex items-center gap-1">
                                                    <svg class="animate-spin w-3 h-3 text-purple-500 shrink-0"
                                        ...
                                        <template x-if="video.status === 'failed'">
                                            <p class="text-xs text-red-500 mt-0.5 truncate" x-text="video.errorMsg">
                                            </p>
                                        </template>
                                    ...
                                    <div class="flex items-center gap-1 shrink-0">
                                        {{-- Retry --}}
                                        <template x-if="video.status === 'failed'">
                                            <button type="button" @click="retry(index)"> {{-- video → index --}}
                    ...
                    <input type="file" x-ref="videoInput" multiple accept="video/mp4,video/quicktime,video/webm"
        ...
        <div x-data="{
            ...
            remove() { this.newFileName = '';
                this.fileSize = '';
                this.removed = true;
                document.getElementById('asset_file_input').value = ''; },
            formatSize(bytes) {
                const units = ['B', 'KB', 'MB', 'GB'];
                let i = 0;
                while (bytes >= 1024 && i < units.length - 1) { bytes /= 1024;
            ...
            <x-drive-upload field-name="drive_file_id" :file-id="old('drive_file_id', $isEdit && $asset->file ? str_replace('drive:', '', $asset->file) : '')" />


            <input type="file" id="file_input" x-ref="fileInput" name="file"
                accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip,.txt,.csv" class="hidden"
    ...
    <div class="space-y-5">
        ...
        {{-- Asset Settings --}}
        <div
            class="rounded-xl border border-gray-100 bg-gray-50/50 p-4 dark:border-gray-700 dark:bg-gray-800/40 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="asset_id_code"
                    value="{{ old('asset_id_code', $isEdit ? $asset->asset_id_code : '') }}" placeholder="AS6748EN"
                    class="shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
            ...
            <div>
                ...
                <select name="status"
                    ...
                    <option value="active"
                        {{ old('status', $isEdit ? $asset->status : 'active') === 'active' ? 'selected' : '' }}>
                        Active</option>
                    <option value="draft"
                        {{ old('status', $isEdit ? $asset->status : '') === 'draft' ? 'selected' : '' }}>Draft
                    </option>
                    <option value="expired"
                        {{ old('status', $isEdit ? $asset->status : '') === 'expired' ? 'selected' : '' }}>
                        Expired</option>
                </select>
                @error('status')
                    <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
