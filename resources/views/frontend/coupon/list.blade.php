                ...
                <tr class="hover:bg-gray-50 transition-colors text-base">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex flex-col">
                            <span class="text-sm  text-blue-600 font-bold tracking-widest" id="cpn-{{ $coupon->id }}">
                                {{ $coupon->slug }}
                            </span>
                            <span class="text-sm text-gray-500 mt-0.5">{{ $coupon->title }}</span>
                        </div>
                    </td>

                    {{-- ২. মেয়াদ (Start to End Date) --}}
                    ...
                    {{-- ৩. মোট ব্যবহার (Used / Total Limit) --}}
                    <td class="px-6 py-4 text-center">
                        <div class="flex flex-col items-center">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-3">
                            {{-- ১. কপি বাটন --}}
                            <button type="button" onclick="copyCouponToClipboard('cpn-{{ $coupon->id }}', this)"
                            ...
                            <button type="button" @click="couponView = 'form'; editingCoupon = @js([
                                ...
                                'name' => $coupon->name,
                                'views' => $coupon->views,
                            ])" class="text-gray-500 hover:text-amber-500 transition-all">
