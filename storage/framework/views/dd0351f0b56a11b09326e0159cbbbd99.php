...
<div class="p-4 mx-auto w-full  md:p-6">
    ...
    <div class="flex flex-col gap-3 mb-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <?php if(auth()->check() && auth()->user()->hasPermission('users.create')): ?>
    ...
    <?php $__currentLoopData = ['success' => 'green', 'error' => 'red']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type => $color): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php if(session($type)): ?>
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
                 class="mb-5 flex items-center gap-3 rounded-lg border border-<?php echo e($color); ?>-200 bg-<?php echo e($color); ?>-50 px-4 py-3 dark:border-<?php echo e($color); ?>-800 dark:bg-<?php echo e($color); ?>-900/20">
                <p class="text-sm font-medium text-<?php echo e($color); ?>-700 dark:text-<?php echo e($color); ?>-400"><?php echo e(session($type)); ?></p>
            </div>
        <?php endif; ?>
    ...
    <div class="rounded-xl border border-gray-100 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]"
        ...
        <div class="flex flex-col gap-3 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400"><?php echo e($users->total()); ?> total users</p>
            </div>
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <form method="GET" action="<?php echo e(route('users.index')); ?>" id="filterForm" class="flex gap-2">
                ...
                <form method="GET" action="<?php echo e(route('users.index')); ?>">
                    ...
                    <?php if(request('role_id')): ?> <input type="hidden" name="role_id" value="<?php echo e(request('role_id')); ?>"> <?php endif; ?>
                    <div class="relative">
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors">
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        <img src="<?php echo e($user->avatar_url ?? asset('images/user/images.png')); ?>" alt="<?php echo e($user->name); ?>"
                                ...
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full capitalize <?php echo e($user->status_badge_class); ?>">
                                ...
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <?php if($user->id !== auth()->id() && !$user->isSuperAdmin()): ?>
                                            <?php if(auth()->check() && auth()->user()->hasPermission('users.delete')): ?>
                                            <button type="button" @click="openDelete(<?php echo e($user->id); ?>, '<?php echo e(addslashes($user->name)); ?>')"
        ...
        <?php if($users->hasPages()): ?>
            <div class="px-6 py-4 border-t border-gray-200 dark:border-white/[0.05]">
                <div class="flex items-center justify-between">
                    ...
                    <?php else: ?>
                        <a href="<?php echo e($users->previousPageUrl()); ?>" class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-3 text-theme-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 sm:px-3.5">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path fill-rule="evenodd" clip-rule="evenodd" d="M2.58301 9.99868C2.58272 10.1909 2.65588 10.3833 2.80249 10.53L7.79915 15.5301C8.09194 15.8231 8.56682 15.8233 8.85981 15.5305C9.15281 15.2377 9.15297 14.7629 8.86018 14.4699L5.14009 10.7472L16.6675 10.7472C17.0817 10.7472 17.4175 10.4114 17.4175 9.99715C17.4175 9.58294 17.0817 9.24715 16.6675 9.24715L5.14554 9.24715L8.86017 5.53016C9.15297 5.23717 9.15282 4.7623 8.85983 4.4695C8.56684 4.1767 8.09197 4.17685 7.79917 4.46984L2.84167 9.43049C2.68321 9.568 2.58301 9.77087 2.58301 9.99715C2.58301 9.99766 2.58301 9.99817 2.58301 9.99868Z" fill="currentColor"/></svg>
                            <span class="hidden sm:inline">Previous</span>
                        </a>
                    <?php endif; ?>
                    <ul class="hidden items-center gap-0.5 sm:flex">
                        <?php $__currentLoopData = $users->getUrlRange(1, $users->lastPage()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $page => $url): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <li>
                                ...
                                <?php else: ?>
                                    <a href="<?php echo e($url); ?>" class="flex h-10 w-10 items-center justify-center rounded-lg text-theme-sm font-medium text-gray-700 hover:bg-blue-500/[0.08] hover:text-blue-500 dark:text-gray-400"><?php echo e($page); ?></a>
                                <?php endif; ?>
                            </li>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </ul>
                    <span class="block text-sm font-medium text-gray-700 dark:text-gray-400 sm:hidden">Page <?php echo e($users->currentPage()); ?> of <?php echo e($users->lastPage()); ?></span>
                    <?php if($users->hasMorePages()): ?>
                        <a href="<?php echo e($users->nextPageUrl()); ?>" class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-3 text-theme-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 sm:px-3.5">
            ...
            <div class="relative w-full max-w-md rounded-xl bg-white p-6 shadow-xl dark:bg-gray-900"
                ...
                <div class="flex flex-col items-center text-center gap-4">
                    ...
                    <div class="flex gap-3 w-full">
                        ...
                        <form :action="'/admin/users/' + deleteId" method="POST" class="flex-1">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\affiliate-project\resources\views/users/index.blade.php ENDPATH**/ ?>