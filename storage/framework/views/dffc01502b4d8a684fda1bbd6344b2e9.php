...
<div class="p-4 mx-auto w-full  md:p-6">
    ...
    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
        ...
        <p class="text-sm font-medium text-red-700 dark:text-red-400"><?php echo e(session('error')); ?></p>
    </div>
    <?php endif; ?>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-lg flex items-center justify-center shrink-0
                                            <?php echo e($role->is_super_admin ? 'bg-amber-100 dark:bg-amber-900/30' : 'bg-blue-100 dark:bg-blue-900/30'); ?>">
                                        <svg class="<?php echo e($role->is_super_admin ? 'text-amber-500' : 'text-blue-500'); ?>" width="18" height="18" viewBox="0 0 20 20" fill="currentColor">
                                            <path fill-rule="evenodd" clip-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" />
                                        </svg>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-900 dark:text-white"><?php echo e($role->label); ?></p>
                                        <p class="text-xs text-gray-400 mt-0.5"><?php echo e($role->name); ?></p>
                                    </div>
                            ...
                            <td class="px-6 py-4">
                                ...
                                <?php else: ?>
                                <span class="text-sm text-gray-700 dark:text-gray-300 font-medium"><?php echo e($role->permissions_count); ?></span>
                                <span class="text-sm text-gray-400"> permissions</span>
                                <?php endif; ?>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-sm text-gray-700 dark:text-gray-300 font-medium"><?php echo e($role->users_count); ?></span>
                                <span class="text-sm text-gray-400"> users</span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <?php if(!$role->is_super_admin): ?>
                                    <?php if(auth()->check() && auth()->user()->hasPermission('roles.edit')): ?>
                                    ...
                                    <?php endif; ?>
                                    <?php if(auth()->check() && auth()->user()->hasPermission('roles.delete')): ?>
                                    <button type="button"
                                        @click="$dispatch('open-delete-modal', { 
            url: '<?php echo e(route('roles.destroy', $role->id)); ?>', 
            title: '<?php echo e(addslashes($role->label)); ?>' 
        })"
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\affiliate\resources\views\roles\index.blade.php ENDPATH**/ ?>